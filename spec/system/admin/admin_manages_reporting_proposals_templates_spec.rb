...
describe "Admin manages proposal answer templates" do # rubocop:disable RSpec/DescribeClass
  let!(:organization) { create(:organization) }
  let!(:user) { create(:user, :admin, :confirmed, organization:) }
  let(:participatory_process) { create(:participatory_process, title: { en: "A participatory process" }, organization:) }
  let!(:reporting_component) { create(:proposal_component, manifest_name: :reporting_proposals, name: { en: "A reporting component" }, participatory_space: participatory_process) }
  let!(:proposals_component) { create(:proposal_component, manifest_name: :proposals, name: { en: "A proposals component" }, participatory_space: participatory_process) }
  let!(:template) { create(:template, target: :proposal_answer, description: { en: description }, field_values:, organization:, templatable: component, skip_injection: true) }
  ...
  describe "using a reporting proposal_answer_template" do
    ...
    context "when displaying current component and organization templates" do
      ...
      let!(:other_component_template) { create(:template, target: :proposal_answer, description: { en: "Foo bar" }, field_values: { internal_state: "evaluating" }, organization:, templatable: other_component) }
      let!(:proposal) { create(:proposal, component:) }
